...
function _getPhotos(files, staticFilesPath, pathFromReq){
  ...
  files = _.map(files, function(photoFileName){
    var objnumber = photoFileName.replace(isPhoto, '').split("_")[0];
    var description = photoFileName.replace(isPhoto, '').split("_")[1];
    var detail = photoFileName.replace(isPhoto, '').split("_")[2];
    if (objnumber.length > 0 && typeof detail !== 'undefined' && detail.length > 0) {
      photoName = description + "_" + detail;
